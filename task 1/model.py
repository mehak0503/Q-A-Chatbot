...
class MemN2N(object):
    ...
    def __init__(self,is_test,nwords,max_words,max_sentences, sess):
        self.nwords = nwords
        self.max_words = max_words
        self.max_sentences = max_sentences
        self.init_mean = 0. #weight initialization mean [0.]
        self.init_std = 0.1 #weight initialization std [0.1]
        self.batch_size = 32
        self.nepoch = 100
        self.anneal_epoch = 25 #lr changes after 25 epochs
        self.nhop = 6
        self.edim = 100 #internal state dimension d[20]
        self.mem_size = 50 #maximum number of sentences that can be encoded into memory [50]
        self.max_grad_norm = 40 #clip gradients to this norm [40]
        
        self.lin_start = False #linear start training
        ...
        if not self.is_test:
            ...
            for idx in range(self.nepoch):
                ...
                state = {
                    ...
                    'val_accuracy':res['test_acc'][-1],
                    'train_accuracy':res['train_acc'][-1]
                }
                ...
                predictions,target = self.predict(train_stories,train_questions)
                cnt = 0
                #print("Target ",target.shape,predictions.shape)
                for i in range(len(target)):
                    if np.argmax(predictions[i])==np.argmax(target[i]):
                        cnt+=1
                res['train_acc'].append(cnt/len(target))
                predictions,target = self.predict(test_stories,test_questions)
                cnt = 0
                #print("Target ",target.shape,predictions.shape)
                for i in range(len(target)):
                    if np.argmax(predictions[i])==np.argmax(target[i]):
                        cnt+=1
                res['test_acc'].append(cnt/len(target))
            plt.plot(res['train_acc'])
            plt.plot(res['test_acc'])
            plt.title('Performance Plot for k=6 and d=100')
            plt.ylabel('accuracy')
            plt.xlabel('epoch')
            plt.legend(['train','test'],loc = 'upper left')
            plt.savefig('k6d100hops.png')
